...
def createMeal(jour, tyrep, itemsName, itemsDict, minItem, maxItem, meanQuantities):
    ...
    foodItems = [Food(itemsDict[x], x, meanQuantities[(itemsDict[x],x)]) for x in items]
...
def createUserDietObject(user, itemsName, itemsDict, minItem, maxItem, nMeal, 
    ...
    if user.sexe_ps == 1:
        quantities = portions['men']
    else:
        quantities = portions['women']
    
    diet = createDiet(itemsName, itemsDict, minItem, maxItem, nMeal, quantities)
